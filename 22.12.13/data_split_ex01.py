...
cat_image_full_path = natsort.natsorted(glob.glob(os.path.join(f"{cat_image_path}/*.jpg"))) #전부 읽어들여라
print("cat image size >> ", len(cat_image_full_path))

dog_image_full_path = natsort.natsorted(glob.glob(os.path.join(f"{dog_image_path}/*.jpg"))) #전부 읽어들여라
print("dog image size >> ", len(dog_image_full_path))

#train 80 val 20 -> val 10 test 10 ->cat
...
#train 80 val 20 -> val 10 test 10 ->dog
